...
void mcloop(TString outfile1 = "", float ptmin = 1, float ptmax = 2, int centmin = 10, int centmax = 20, float ymin = -3, float ymax = 3)
{
    ...
    newtree->Branch("nmatchGEN", &nmatchGEN, "nmatchGEN[nDsize]/O");
    newtree->Branch("nisSwap", &nisSwap, "nisSwap[nDsize]/O");
    ...
    while (true)
    {
        ...
        cout << ifile << " / 5603      " << 100 * ifile / 5603 << "%" << endl;
        cout << "-- -- -- -- -- --" << endl;
        }
        if (istest && ifile>50) break;
        ...
        TTree *tree = (TTree *)fin->Get("d0ana_newreduced/VertexCompositeNtuple");
       // TTree *friendTree = (TTree *)fin->Get("eventinfoana/EventInfoNtuple");
        ...
        // for (int i=0; i<3000; i++)
        {
            ...
            {
                if (TMath::IsNaN(x3DDecayLengthSignificance[j]))
                    continue;
                if (TMath::IsNaN(nxyDCASigD2[j]))
                    continue;
                if (TMath::IsNaN(nzDCASigD2[j]))
                    continue;
                if (abs(y[j]) < ymin || abs(y[j]) >= ymax)
                    continue;
                if (matchGEN[j]!=1) continue;
                if (pT[j] < ptmin || pT[j] >= ptmax) continue;
                if (abs(y[j]) >= ymin && abs(y[j]) < ymax && matchGEN[j] == 1 && isSwap[j] ==0 && pT[j] >= ptmin && pT[j] < ptmax)
                {
...
{
    if (argc == 8) // Expecting 5 arguments + program name
    {
        ...
        float ymax = atoi(argv[7]);

        mcloop(outfile1, ptmin, ptmax, centmin, centmax, ymin, ymax);
    }
    else
    {
        std::cout << "Usage: mergeForest <input_collection> <output_file> <ptmin> <ptmax> <centmin> <centmax> <ymin> <ymax>" << std::endl;
        return 1;
    }
    return 0;
}
