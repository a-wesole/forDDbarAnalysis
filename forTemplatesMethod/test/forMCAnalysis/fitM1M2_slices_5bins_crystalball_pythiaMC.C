...
void fitM1M2_slices_5bins_crystalball_pythiaMC()
{
  ...
  ratioGraphs.push_back(gr_ratio);
    ...
    // step - for each slice of M1
    // for (int i = 0; i < number_of_slices; i++)
    // // for (int i = 0; i < 1 ; i++)
    // {
      // after you have created/filled F1_bin, fitted_signal, fitted_swap, fitted_background, M1_canvas, etc.
      ...
      // TF1 *fitted_foreground_slice = nullptr;
      // TF1 *fitted_signal_slice = nullptr;
      // TF1 *fitted_swap_slice = nullptr;
      // TF1 *fitted_background_slice = nullptr;
      ...
      // // step - first fit F1_reference to the slice and only release an overall scaling factor and the signal/swap ratio
      // scale1 = 0.0;
      // ratio = 0.0;
      // fitted_foreground_reference = (TF1 *)F1_bin->Clone("f31_protected");

      // fitted_foreground_slice = new TF1(Form("F1_bin%d_slice%d", dphi_bin, i), [fitted_foreground_reference](double *x, double *p)
      //                                   {fitted_foreground_reference->SetParameter(5, p[1]); return p[0] * fitted_foreground_reference->Eval(x[0]); }, fit_range_low, fit_range_high, 2);

      // fitted_foreground_slice->SetParameter(0, 1.0);
      // fitted_foreground_slice->SetParameter(1, fitted_foreground_reference->GetParameter(5));
      // // fitted_foreground_slice->SetParameter(1, fitted_foreground_reference->GetParameter(5));
      // fitted_foreground_slice->SetParLimits(1, 0.0, 1.0);
      // M1M2hx->Fit(fitted_foreground_slice, "RQ0"); // quiet, no draw
      // M1M2hx->Fit(fitted_foreground_slice, "RQ0"); // quiet, no draw
      // M1M2hx->Fit(fitted_foreground_slice, "RQ0"); // quiet, no draw
      // cout << "Slice " << i + 1 << ": scale factor from fit = " << fitted_foreground_slice->GetParameter(0) << endl;
      // cout << "parameter 5 from fit = " << fitted_foreground_slice->GetParameter(1) << endl;
      // scale1 = fitted_foreground_slice->GetParameter(0);
      // ratio = fitted_foreground_slice->GetParameter(1);

      // // step - apply the same scale1 and ratio to the fitted_signal_slice, fitted_swap_slice, and fitted_background_slice

      // fitted_signal_slice = new TF1(Form("fitted_signal_slice_%d", i), [fitted_signal](double *x, double *p)
      //                               {fitted_signal->SetParameter(5, p[1]); return p[0] * fitted_signal->Eval(x[0]); }, fit_range_low, fit_range_high, 2);
      // fitted_signal_slice->FixParameter(0, scale1);
      // fitted_signal_slice->FixParameter(1, ratio);

      // fitted_swap_slice = new TF1(Form("fitted_swap_slice_%d", i), [fitted_swap](double *x, double *p)
      //                             {fitted_swap->SetParameter(5, p[1]); return p[0] * fitted_swap->Eval(x[0]); }, fit_range_low, fit_range_high, 2);
      // fitted_swap_slice->FixParameter(0, scale1);
      // fitted_swap_slice->FixParameter(1, ratio);

      // fitted_background_slice = new TF1(Form("fitted_background_slice_%d", i), [fitted_background](double *x, double *p)
      //                                   { return p[0] * fitted_background->Eval(x[0]); }, fit_range_low, fit_range_high, 1);
      // fitted_background_slice->FixParameter(0, scale1);

      // M1M2hx->SetLineColor(kGray);
      // M1M2hx->SetMarkerColor(kGray);
      // M1M2hx->SetMarkerStyle(20);
      // M1M2hx->SetMinimum(0);

      // M1M2hx->Draw("E");
      // fitted_foreground_slice->SetLineColor(kRed);
      // fitted_foreground_slice->Draw("same");
      // fitted_signal_slice->SetLineColor(kBlue);
      // fitted_signal_slice->Draw("same");
      // fitted_swap_slice->SetLineColor(kGreen);
      // fitted_swap_slice->Draw("same");
      // fitted_background_slice->SetLineColor(kBlack);
      // fitted_background_slice->Draw("same");

      // M1_canvas->Update();
      // M1_canvas->Print(pdfName);

      // cout << "M1M2hx nBinsX = " << M1M2hx->GetNbinsX() << endl;
      // cout << "signalMC nBinsX = " << signalMC->GetNbinsX() << endl;
      // cout << "swapMC nBinsX = " << swapMC->GetNbinsX() << endl;
      // cout << "bkgMC nBinsX = " << bkgMC->GetNbinsX() << endl;

      // results->cd();
      // fitted_foreground_slice->Write(Form("F1_bin%d_slice%d", dphi_bin, i));
      // fitted_signal_slice->Write(Form("S1_bin%d_slice%d", dphi_bin, i));
      // fitted_swap_slice->Write(Form("SW1_bin%d_slice%d", dphi_bin, i));
      // fitted_background_slice->Write(Form("B1_bin%d_slice%d", dphi_bin, i));

      // F_getHistogram = (TH1 *)fitted_foreground_slice->GetHistogram()->Clone(Form("F1histo_bin%d_slice%d", dphi_bin, i));
      // signal_getHistogram = (TH1 *)fitted_signal_slice->GetHistogram()->Clone(Form("S1histo_bin%d_slice%d", dphi_bin, i));
      // swap_getHistogram = (TH1 *)fitted_swap_slice->GetHistogram()->Clone(Form("SW1histo_bin%d_slice%d", dphi_bin, i));
      // background_getHistogram = (TH1 *)fitted_background_slice->GetHistogram()->Clone(Form("B1histo_bin%d_slice%d", dphi_bin, i));
